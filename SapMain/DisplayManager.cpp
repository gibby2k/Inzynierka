...
void updateDisplayClock() {
    if (millis() - lastTick > 1000) {
        lastTick = millis();
        colonState = !colonState; // Miganie dwukropka

        struct tm timeinfo;
        if (getLocalTime(&timeinfo, 10)) {
            // Pomyślnie pobrano czas atomowy z systemu
            simHour = timeinfo.tm_hour;
            simMinute = timeinfo.tm_min;
            simSecond = timeinfo.tm_sec;
        } else {
            // TRYB AWARYJNY (Fallback) - Jeśli brak WiFi, zegar sam liczy czas
            simSecond++;
